...
def make_one_image_sample(path):
    image_pil = Image.open(path)
    img_data = np.array(image_pil)
    img_data.astype(np.float32)
    img_data = img_data / 255
    # img_3d = np.stack((img_data,) * 3, axis=-1)

    print(img_data.shape)

if __name__ == "__main__":
    test_sample = "./sample/TestAll/double (9).jpg"
    make_one_image_sample(test_sample)
